...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if created:
            self.stdout.write(self.style.SUCCESS('Group "Модератор" created'))
        else:
        ...
        for perm in permissions:
            ...
            self.stdout.write(f'Permission {perm.codename} added to group "Модератор"')

        self.stdout.write(
            self.style.SUCCESS('Permissions assigned to group "Модератор"')
        )
